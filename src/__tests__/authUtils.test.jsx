...
import {
  ...
  handleAuthRedirect,
  onPlanClickHandler,
} from "../utils/authutils"; // Adjust path as needed
  ...
  describe("handleAuthRedirect", () => {
    ...
    test("redirects to document verification if the document is not verified", async () => {
      ...
      expect(setAlertMessage).toHaveBeenCalledWith(
        "Redirecting to the Document Verification Page..."
      );
    });
    ...
    test("redirects to plan confirmation if the document is verified", async () => {
      ...
      expect(setAlertMessage).toHaveBeenCalledWith(
        "Redirecting to the Plan Confirmation Page..."
      );
    });
    ...
    test("handles errors during document verification", async () => {
      ...
      expect(setAlertMessage).toHaveBeenCalledWith(
        "Error occurred during the process. Please try again."
      );
    });
  });
