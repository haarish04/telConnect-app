...
describe("DocumentVerification Component", () => {
  ...
  test("renders correctly", () => {
    renderComponent();
    expect(screen.getByText(/Please upload documents to complete KYC/i)).toBeInTheDocument();
  ...
  test("shows an error message for file size exceeding limit", () => {
    ...
    const fileInput = screen.getByLabelText(/Upload File:/i);
    fireEvent.change(fileInput, {
      target: { files: [new File(["a".repeat(501 * 1024)], "large.pdf", { type: "application/pdf" })] },
    });
  ...
  test("uploads a valid document and shows success message", async () => {
    ...
    const fileInput = screen.getByLabelText(/Upload File:/i);
    fireEvent.change(fileInput, {
      target: { files: [new File(["valid content"], "document.pdf", { type: "application/pdf" })] },
    });
  ...
  test("shows an error message when document verification fails", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/Document submission failed. Please try again/i)).toBeInTheDocument();
    });
  });
});
